...
<div class="container my-3">
        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-end">
                ...
                <div class="badge bg-light text-dark p-2 text-start">
                    ...
                    <div>{question.create_date ? moment(question.create_date).format("YYYY년 MM월 DD일 hh:mm a") : ''}</div>
                </div>
            </div>
            <div class="my-3">
    ...
    <!-- 답변 목록 -->
    <h5 class="border-bottom my-3 py-2">{question.answers ? question.answers.length : 0}개의 답변이 있습니다.</h5>
    ...
    <!-- 답변 등록 -->
    <Error error={error} />
<form on:submit={post_answer} class="my-3">
    <div class="mb-3">
        <!-- 오타 수정, disabled 바인딩 간소화, placeholder 추가, 태그 정상 닫기 -->
        <textarea 
            rows="10" 
            bind:value={content} 
            disabled={!$is_login} 
            class="form-control" 
            placeholder={$is_login ? "답변을 입력하세요." : "로그인 후 답변을 작성할 수 있습니다."}
        ></textarea>
    </div>
    <input type="submit" value="답변등록" class="btn btn-primary" disabled={!$is_login} />
</form>
</div>
