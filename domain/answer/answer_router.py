...
# 🛠️ 1. status_code 오타 수정 (HTTP_244 -> HTTP_204)
...
# 🛠️ 2. 답변 삭제 방식을 Query Parameter(answer_id: int)로 전송 규격 변경
# domain/answer/answer_router.py

@router.delete("/delete", status_code=status.HTTP_204_NO_CONTENT)
def answer_delete(answer_id: int,  # 👈 기존 스키마를 지우고 일반 자료형(int)으로 변경!
                  db: Session = Depends(get_db),
                  current_user: User = Depends(get_current_user)):
    # ⚠️ 내부 변수 참조할 때 _answer_delete.answer_id 대신 매개변수 answer_id를 바로 씁니다.
