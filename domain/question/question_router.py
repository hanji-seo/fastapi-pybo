...
# 수정 후 백엔드 코드 (방법 1이 안 될 때 우회하는 법)
from fastapi import Depends  # 상단 Depends 확인


@router.delete("/delete", status_code=status.HTTP_204_NO_CONTENT)
def question_delete(_question_delete: question_schema.QuestionDelete = Depends(),  # 👈 뒤에 = Depends()를 붙여줍니다!
                    db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
